...
\text{where} \qquad \mathbf{f} & \epsilon \mathbf{R^m}. \\
\end{aligned}
\end{equation}
...
Supervised learning is a subgroup of machine learning which is based on labelled data.
...
\cite{Primartha2018} \cite{Paul2018} \cite{Shi2006}

\subsection{Support Vector Machines}
Support vector machines (SVM) creates a hyperplane between the feature space to increase the margin between the data samples nearest to the hyperplane from all the classes within the data \cite{hearst1998support, meyer2015support}. The hyperplane can be defined as
\begin{equation}
...
\end{equation}
This is provided with the example of a polynomial division of the data shown in Figure~\ref{fig:SupportVectorMachinesExample} with a polynomial fit as shown in Figure~\ref{fig:SupportVectorMachinesExamplePoly}.
